...
data_folder = p.cwd() / "generated_id_images"
...
def display_images_in_grid(images):
    ...
    for i, ax in enumerate(axes.flat):
        if i < len(images):
            ...
            target_file = os.path.join("generated_id_images/", filename)  # Adjust the new filename if needed
            ...
            ax.axis("off")
###############################################################################
def image_creation(image_prompt):
    if image_prompt!="":
